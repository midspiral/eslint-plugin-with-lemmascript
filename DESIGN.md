...
**Status:** _spike verified (2026-06-01)._ Stage 0 (reachability core) **and** the Stage 1
domination/strictness meta-theorem are proved — **23 Dafny obligations, 0 errors**, and
`src/core.verified.ts` typechecks. The ESLint shell + demo are still to come.
...
**First action — the spike (✓ done 2026-06-01):** Stage 0 + the strictness witness from
Stage 1. LemmaScript proves `canReach` sound+complete+terminating, `reachesAny`/`violates`
exact, and exhibits `violates ∧ ¬directViolation` on the 2-edge laundered path `ui→svc→db`
(`Strictness`). 23 obligations, 0 errors; the core typechecks. The proof is clean and
followable — the reachability machinery is the proven xyflow `canReach`, the fresh content
is the `Domination`/`Strictness` pair. The spine holds; the plugin builds around it. **Next:
Stage 3 — the ESLint shell** (graph extraction + the `no-forbidden-reach` rule).
